...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="collection/record">
            ...
            <xsl:for-each
                select="
                    datafield[@tag = '020']/subfield[@code = 'a'][contains(., '(') and
                    contains(upper-case(.), 'PAPER')
                    or contains(upper-case(.), 'HARD')
                    or contains(upper-case(.), 'PB')
                    or contains(upper-case(.), 'CLOTH')
                    or contains(upper-case(.), 'CL.')
                    or contains(upper-case(.), 'BOUND')
                    or contains(upper-case(.), 'HBK')
                    or contains(upper-case(.), 'HB')
                    or contains(upper-case(.), 'HC')
                    or contains(upper-case(.), 'HDB')
                    or contains(upper-case(.), 'HD')
                    or contains(upper-case(.), 'TRADE')]">
                <!-- Batch is global variable -->
                <xsl:variable name="flag">
                ...
                <xsl:choose>
                    <xsl:when
                        test="
                            ...
                            or @ind2 = ' '
                            or @ind2 = '0'
                            or (@ind1 = '4' and @ind2 != '1' and contains(upper-case(./*), 'CONTENTS'))
                            or contains(upper-case(./*), 'INHALTSVERZEICHNIS')
                            or contains(upper-case(./*), 'INHALTSVERZEICHNIS')
                            ">
                        <!-- Batch is global variable -->
                        <xsl:variable name="flag">
                        ...
                        <xsl:variable name="details">
                            <xsl:value-of
                                select="concat('Ind1:', $ind1, ' ; ', 'Ind2:', $ind2, ' ; ', '$3: ', $url3, '$3: ', $urlY, '$z: ', $urlZ)"
                            />
                        </xsl:variable>
                        <xsl:value-of
                            select="concat($batch, $delimiter, $flag, $delimiter, $mms, $delimiter, $this_oclc, $delimiter, $details, $delimiter, normalize-space($this_title), '&#13;')"
