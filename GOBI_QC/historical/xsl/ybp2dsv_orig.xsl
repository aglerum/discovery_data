    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="*/marc:record">
            ...
            <xsl:variable name="batch">
                <xsl:value-of select="'YBP-APPROVAL_ap16190102'"/>
            </xsl:variable>
            ...
            <xsl:for-each select="marc:datafield[@tag = '020']/marc:subfield[@code = 'a']">
                ...
                <xsl:choose>
                    <xsl:when
                        test="
                            (marc:subfield[@code = 'a'] and marc:datafield[@tag = '020']/marc:subfield[@code = 'a'][not(following-sibling::marc:subfield[@code = 'q'])])
                            and
            ...
            <xsl:choose>
                <xsl:when
                    test="
                        ($titleInd1 eq '0' and marc:datafield[@tag = '100'] or marc:datafield[@tag = '110'] or marc:datafield[@tag = '111'] or marc:datafield[@tag = '130'])
            ...
            <xsl:choose>
                <xsl:when test="marc:datafield[@tag = '264'][ind = '4'] and not(starts-with(., '©') or starts-with(., 'copyright'))">
                    <rows>
                        <column>
                            <xsl:value-of select="$batch"/>
                        </column>
            ...
            <xsl:for-each select="marc:datafield[@tag = '245']">
                <xsl:choose>
                    <xsl:when
                        test="
                            not(marc:datafield[@tag = '993']) and
