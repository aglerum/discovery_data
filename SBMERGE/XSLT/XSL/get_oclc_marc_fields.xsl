...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:text>035&#x9;019&#x9;007pos0&#x9;008form&#x9;300&#x9;336&#x9;338&#x9;533&#x9;538&#x9;776$i$w&#x9;856&#13;</xsl:text>
        <!-- Tab Delimiter -->
        ...
        <xsl:for-each select="marc:collection/marc:record">
            ...
            <xsl:variable name="field_019">
                <xsl:for-each select="marc:datafield[@tag = '019']/marc:subfield[@code = 'a']">
                    <xsl:value-of select="."/>
                    <xsl:text>; </xsl:text>
                </xsl:for-each>
            </xsl:variable>
            ...
            <xsl:variable name="field_533">
                <xsl:value-of select="marc:datafield[@tag = '533']"/>
            </xsl:variable>

            <!-- 538 field -->
            <xsl:variable name="field_538">
                <xsl:value-of select="marc:datafield[@tag = '538']"/>
            </xsl:variable>
            ...
            <xsl:variable name="field_856">
                <xsl:for-each select="marc:datafield[@tag = '856'][@ind1='4'][@ind2='0']">
            ...
            <xsl:value-of
                select="concat(normalize-space($field_035), $delimiter, normalize-space($field_019), $delimiter, $field_007, $delimiter, $field_008, $delimiter, normalize-space($field_300), $delimiter, normalize-space($field_336), $delimiter, normalize-space($field_338), $delimiter, normalize-space($field_533), $delimiter, normalize-space($field_538), $delimiter, $field_776, $delimiter, $field_856, '&#13;')"
            />
        </xsl:for-each>
    </xsl:template>
</xsl:stylesheet>
